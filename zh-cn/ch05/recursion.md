...
处理数字时也与之相似。函数一般都得接受一个值并修改它。早些时候我们编写过一个计算 Fibonacci 的函数，它便是某数与它减一的 Fibonacci 数的积。让它乘以零就不行了， Fibonacci 数又都是非负数，边界条件便可以定为 1，即乘法的单位元。 因为任何数乘以 1 的结果还是这个数。而在 ``sum`` 中，加法的单位元就是 0。在快速排序中，边界条件和单位元都是空 List，因为任一 List 与空 List 相加的结果依然是原 List。
