...
我们深入看下 ``fold`` 的执行过程：``\acc x-> acc + x`` 是个二元函数，``0`` 是初始值，``xs`` 是待折叠的 List。一开始，累加值为 ``0``，当前项为 ``3``，呼叫二元函数 ``0+3`` 得 ``3``，作新的累加值。接着来，累加值为 ``3``，当前项为 ``5``，得新累加值 ``8``。再往后，累加值为 ``8``，当前项为 ``2``，得新累加值 ``10``。最后累加值为 ``10``，当前项为 ``1``，得 ``11``。恭喜，你完成了一次折叠 ``(fold)``！
