...
这个函数的型别会是什么样呢？我们希望他接受一个字串当作参数，并产出一个数值作为结果。所以应该会是 ``solveRPN :: (Num a) => String -> a``。
    ...
    小建议：在你去实作函数之前，先想一下你会怎么声明这个函数的型别能够帮助你厘清问题。在 Haskell 中由于我们有够强的型别系统，光从函数的声明就可以得到许多信息。
...
另外注意到我们加了 ``Read a`` 这像 class constraint，毕竟我们要使用到 ``read`` 来转换成数值。所以我们必须要声明成他要属于 ``Num`` 跟 ``Read`` 两种 typeclass。（譬如说 ``Int``,``Float`` 等）
...
我们来改写一下我们的函数让他多支持几个运算子。为了简单起见，我们改写声明让他回传 ``Float`` 型别。
...
我们现在要做的就是用 Haskell 实作我们先前的解法。所以我们应该怎样声明我们计算最短路径函数的型别呢？他应该接受一个道路系统作为参数，然后回传一个路径。我们会用一个 list 来代表我们的路径。我们定义了 ``Label`` 来表示 ``A``, ``B`` 或 ``C``。并且也定义一个同义词 ``Path``：
